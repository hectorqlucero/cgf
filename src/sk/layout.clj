...
(defn menus-none []
  (list
   [:nav.navbar.navbar-expand-md.navbar-light.fixed-top {:style "background-color: #e3f2fd;"}
    ...
    [:div#collapsibleNavbar.collapse.navbar-collapse
     [:ul.navbar-nav
      [:li.nav-item [:a.nav-link {:href "/somos"} "QUIENES SOMOS"]]
      [:li.nav-item [:a.nav-link {:href "/ventas"} "VENTAS Y SERVICIOS"]]
      [:li.nav-item [:a.nav-link {:href "/asistencia"} "ASISTENCIA TECNICA"]]]]]))
